      ...
      <section className="dashboard-card p-6 sm:p-8">
        <div className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <div>
            ...
            <h1 className="mt-4 text-4xl font-serif text-[#1F6F5F] sm:text-5xl">Find the right expert without leaving your workflow.</h1>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-[#1F6F5F]/65">
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              ...
              <div key={item.label} className="rounded-[28px] border border-[#1F6F5F]/10 bg-[#EEEEEE]/80 p-5">
                <p className="text-2xl font-semibold text-[#1F6F5F]">{item.value}</p>
                <p className="mt-2 text-sm text-[#1F6F5F]/55">{item.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="dashboard-card p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            {tabs.map((tab) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-semibold transition ${
                  activeTab === tab ? 'bg-[#1F6F5F] text-white' : 'bg-[#EEEEEE] text-[#1F6F5F]/65 hover:bg-[#6FCF97]/20'
                }`}
              >
                {tab}
              </button>
          ...
          <div className="relative w-full max-w-md">
            <Search className="pointer-events-none absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-[#1F6F5F]/45" />
            <input
              type="text"
              value={query}
        ...
        {isLoading && <p className="mt-6 text-sm text-[#1F6F5F]/55">Loading vet network...</p>}
        {error && <p className="mt-6 text-sm text-[#1F6F5F]">{error}</p>}

        <div className="mt-6 grid gap-5 lg:grid-cols-2 2xl:grid-cols-3">
          {filteredDoctors.map((doctor) => (
            <article key={doctor.id} className="rounded-[30px] border border-[#1F6F5F]/10 bg-[#EEEEEE]/75 p-6 transition hover:-translate-y-1 hover:border-[#6FCF97]/50 hover:shadow-[0_24px_60px_rgba(15,23,42,0.07)]">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-4">
                  <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#1F6F5F] text-lg font-semibold text-[#6FCF97]">
                    {getInitials(doctor.name)}
                  </div>
                  <div>
                    <h2 className="text-xl font-semibold text-[#1F6F5F]">{doctor.name}</h2>
                    <p className="mt-1 text-sm text-[#1F6F5F]">{doctor.spec}</p>
                  </div>
                </div>
                <span className={`rounded-full px-3 py-1 text-xs font-semibold ${
                  doctor.avail === 'Available Today' ? 'bg-[#6FCF97]/24 text-[#1F6F5F]' : 'bg-[#EEEEEE] text-[#1F6F5F]'
                }`}>
                  {doctor.avail}
                </span>
              </div>

              <div className="mt-6 grid gap-3 text-sm text-[#1F6F5F]/65">
                <div className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3">
                  <MapPin className="h-4 w-4 text-[#1F6F5F]" />
                  {doctor.city}
                </div>
                <div className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3">
                  <Stethoscope className="h-4 w-4 text-[#1F6F5F]" />
                  {doctor.exp} years of experience
                </div>
                <div className="flex items-center gap-3 rounded-2xl bg-white px-4 py-3">
                  <Video className="h-4 w-4 text-[#1F6F5F]" />
                  {doctor.mode}
                </div>
              </div>
              ...
              <div className="mt-6 flex items-center justify-between">
                <div className="flex items-center gap-2 rounded-full bg-white px-3 py-2">
                  <Star className="h-4 w-4 fill-[#6FCF97] text-[#6FCF97]" />
                  <span className="text-sm font-semibold text-[#1F6F5F]">{doctor.rating}</span>
                </div>
                <div className="flex items-center gap-2 text-xs font-medium text-[#1F6F5F]/55">
        ...
        {!isLoading && !error && filteredDoctors.length === 0 && (
          <div className="mt-6 rounded-3xl border border-dashed border-[#1F6F5F]/10 bg-[#EEEEEE] px-6 py-10 text-center text-sm text-[#1F6F5F]/55">
